...
class LinearWorker(BaseWorker):
    ...
    def _build_ffmpeg_args(self, concat_file: Path, input_path: Path) -> List[str]:
        ...
        args = ["ffmpeg", "-re", "-fflags", "+genpts", "-threads", "2"]
        ...
        if self.config.data["output"]["mode"] in ("http_hls", "ram_http"):
            ...
            playlist_filename = str(hls_path.resolve() / "index.m3u8").replace("\\", "/")
            
            # Get encoding arguments from the service
            ...
            # Handle subtitles
            if self.config.data.get("ffmpeg", {}).get("transcoding", {}).get("enabled") is True:
                # Add subtitle mapping logic here if needed for concat
                pass
            else:
